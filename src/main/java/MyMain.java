public class MyMain {
    ...
    public static int sumDigits(int x) {
        ...
        int res = 0;
        while(num>0) {
            res += num % 10; //add last digit to result
    ...
    public static boolean isSelfDivisor(int num) {
        if(String.valueOf(num).contains("0")) return false; //if contains 0 return false
        int x = num; //x is just the input but mutable. honestly don't know if this is necessary
        int res = 0; //result var
        while(x>0) {
            if(!(num % (x % 10) == 0)) return false; //test for divisibility by last digit
            x /= 10; //cut off last digit
        }
        return true;
    }
